...
void Horizontal8Bands(void){
    fillScreen(BLACK);
    //Horizontal bands must traverse the height
    int bandLocation = HEIGHT;
    int i = 0;
            while (bandLocation >= 0){
                //draw several lines to make a thicker "band"
                while (i < 7){
                    drawFastHLine(0,bandLocation + i, WIDTH, GREEN);
                    i++;
                }
                i = 0;
                bandLocation = bandLocation - 16; //subtract 16 because we know 128/8 = 16
                delay(70);
...
void Vertical8Bands(void){
    fillScreen(BLACK);
    //Vertical bands must traverse the width
    int bandLocation = WIDTH;
    int i = 0;
            while (bandLocation >= 0){
                //draw several lines to make a thicker "band"
                while (i < 7){
                    drawFastVLine(bandLocation + i,0, HEIGHT, BLUE);
                    i++;
                }
                i = 0;
                bandLocation = bandLocation - 16; //subtract 16 because we know 128/8 = 16
