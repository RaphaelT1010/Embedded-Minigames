...
void writeCommand(unsigned char c) {
    ...
    //3. Write OC Low (Pin 60) to indicate CS enabled
    GPIOPinWrite(GPIOA0_BASE, 0X20, 0);

    //4. Put data command c into OLED
    SPIDataPut(GSPI_BASE, c);

    //5. Clean up register with the dummy
    SPIDataGet(GSPI_BASE,&addr);

    //6. Write OC High (Pin 60) to indicate CS disabled
    GPIOPinWrite(GPIOA0_BASE, 0X20, 0xFF);
